...
- **CQ API** (`popup_sends[]`) — тайминги и названия отправленных попапов из карточки
  пользователя. Два попапа, отправленные с разницей в пару секунд, — сигнал бага даже без
  визуального DOM-наложения. **Важно:** API отдаёт события под ключом `data` (не `events`),
  `user_id` забирается в конце сессии с ретраями (`get_cq_user_id`).
