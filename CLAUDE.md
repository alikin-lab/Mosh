...
agent/
  ...
  summary.py         — агрегатор отчётов в таблицу (помечает фабрикации)
...
`SUMMARY.md` → печатает `ИТОГ · run_token=...`. Папка по умолчанию — `reports/<host>`.
Это единственная команда, которую запускает агент-тестировщик: её stdout = доказательство
реального прогона.
